...
def generate_masks(mask_path, batch_size):             # for optical filters-based system
    mask = sio.loadmat(mask_path + '/mask_3d.mat')
...
# def generate_masks(mask_path, batch_size):            # for CASSI system
...
def gen_meas_torch(data_batch, mask3d_batch, Y2H=True, mul_mask=False):     
    ...
    temp = shift(mask3d_batch * data_batch, 0)
    meas = torch.sum(temp, 1)
    [bs, row, col] = meas.shape
    nC = 28
    if Y2H:
        meas = meas / nC * 2
        H = shift_back(meas, 0)           # for optical filters-based system, shifting step=0; for CASSI system, shifting step=2.
        if mul_mask:
